...
//scanIntoMap 如果是map 情况数据库查出什么字段 就是什么字段， 自己注意大小写处理
...
func Scan(rows *sql.Rows, db *DB, initialized bool) {
	//isToLowercase := db.Dialector.Name() == db.Config.ScanColumnsToLowercaseDialector
	columns, _ := rows.Columns()
	values := make([]interface{}, len(columns))
	db.RowsAffected = 0
	switch dest := db.Statement.Dest.(type) {
	case map[string]interface{}, *map[string]interface{}: //转成map 或者 指针map 情况
	...
	case *[]map[string]interface{}: //转成map 情况
		columnTypes, _ := rows.ColumnTypes()
...
//func getColumns(rows *sql.Rows, isToLowercase bool) []string {
//	columns, _ := rows.Columns()
//	if isToLowercase {
//		var results []string
//		for i := 0; i < len(columns); i++ {
//			results = append(results,strings.ToLower(columns[i]))
//		}
//		return results
//	}
//    return columns
